...
public class RidgedMultifractalNoise
{
    ...
    private const int _maxOctaves = 8;
    ...
    private int _seed;
    private readonly double[] _weights = new double[_maxOctaves];
    ...
    public int OctaveCount
    {
        ...
        set { _octaveCount = Mathf.Clamp(value, 1, _maxOctaves); }
    }
    ...
    public double GetValue(double x, double y, double z)
    {
        ...
        {
            var nx = MakeInt32Range(x);
            var ny = MakeInt32Range(y);
            var nz = MakeInt32Range(z);
            long seed = (_seed + i) & 0x7fffffff;
            var signal = GradientCoherentNoise3D(nx, ny, nz, seed);
    ...
    private double GradientCoherentNoise3D(double x, double y, double z, long seed)
    {
    ...
    private double InterpolateLinear(double a, double b, double position)
    {
